...
if __name__ == "__main__":
    ...
    df_gas_fee = pd.read_csv(
        ...
        + "/gas_fee/data_source/etherscan_avg_gas_price.csv",
        index_col="Date(UTC)",
    )
    ...
    df_eth_price = pd.read_csv(
        config["dev"]["config"]["data"]["GLOBAL_DATA_PATH"]
        + "/gas_fee/data_source/etherscan_ether_price.csv",
        index_col="Date(UTC)",
    )
    ...
    file_name = path.join(
        config["dev"]["config"]["data"]["GLOBAL_DATA_PATH"], "gas_fee/avg_gas_fee.csv"
    )
