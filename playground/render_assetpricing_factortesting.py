...
if __name__ == "__main__":
    dom_variable = "volume_ultimate_share"
    n_quantiles = 3
    # load factors
    ...
    # Get the regression panel dataset from pickled file
    data_panel = pd.read_pickle(
        PROCESSED_DATA_PATH / "panel_main.pickle.zip", compression="zip"
    )

    # clean data, which will be different for each dominant variable
    reg_panel = clean_weekly_panel(
        data_panel, dom_variable, is_stablecoin=0, is_boom=-1
    )
    summary_table = univariate_sort(reg_panel, dom_variable, n_quantiles)

    # Compute the difference: H minus L
    pivot = reg_panel.pivot_table(index="WeekYear", columns="portfolio", values="ret")
    pivot["CDOM"] = pivot[f"P{n_quantiles}"] - pivot["P1"]
    dominance_factor = pivot[["CDOM"]].reset_index()
    assets_panel = clean_weekly_panel(
        data_panel, dom_variable, is_stablecoin=0, is_boom=-1
    )
    # merge all factors
    ...
    dvs = ["CDOM"]
    ivs = [["MKT"], ["MKT", "HML", "SMB"], ["CMKT"], ["CMKT", "CSIZE", "CMOM"]]
    ...
    reg_panel = lag_variable_columns(reg_panel, dvs + ivs_unique, "WeekYear", "Token")
    ...
    result_one_column = render_regression_column(
        ...
        dv=reg_combi[0][0],
        iv=reg_combi[0][1],
        panel_index_columns=(["Token", "WeekYear"], [True, False]),
        standard_beta=True,
    )
