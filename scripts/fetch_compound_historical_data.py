...
if __name__ == "__main__":
  ...
  compound_assets = {'ETH': '0x4ddc2d193948926d02f9b1fe9e1daa0718270ed5',
  ...
  df_compound_assets = pd.DataFrame(list(compound_assets.items()), columns=['underlying_symbol', 'ctoken_address'])
  ...
  for index, row in df_compound_assets.iterrows():
    ...
    supply_result, borrow_result, price_result, exchange_rate_result = fetch_asset_historical_data(ctoken_address, start_timestamp, end_timestamp, horizon)
    ...
    df_asset_history = pd.DataFrame({'ctoken_symbol': ctoken_symbol, 
                                    'date': date_list, 
