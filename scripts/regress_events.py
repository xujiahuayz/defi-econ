...
reg_panel["treat"] = (reg_panel["Date"] >= reg_panel["join_compound_time"]).astype(
    float
)
...
reg_panel[TIME_TO_JOIN] = (
    reg_panel["Date"] - reg_panel["join_compound_time"]
).dt.days.map(
...
did_reg_panel = reg_panel[
    ["Token", "Date", "join_compound_time", TIME_TO_JOIN] + DEPENDENT_VARIABLES
].merge(
