...
def get_all_pools_v3() -> pd.DataFrame:
    """
  ...
  """
    ...
    get_pairs_batch0 = subgraph.run_query(subgraph.http_v3, get_pairs_query_0)

    # Create a dataframe to store the pairs info, initializa with the first 1000 pairs
    df_all_pairs = pd.DataFrame.from_dict(get_pairs_batch0["data"]["pools"])

    # Do iteration to fetch all the pairs, skip the first 1000 in batch 0
    # Start from the last timestamp which is got by batch 0

    iter_count = 0
    pairs_iter = get_pairs_batch0["data"]["pools"]  # Pair info list in each batch
    ...
    """
        ...
        pairs_iter = result_iter["data"]["pools"]
