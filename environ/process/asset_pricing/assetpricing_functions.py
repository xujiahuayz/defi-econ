...
def clean_weekly_panel(reg_panel, dom_variable, is_stablecoin=0, is_boom=-1):
    ...
    #     lambda group: group["mcap"].max() >= 1e6
    # )
...
def univariate_sort(df_panel, dom_variable, n_quantiles=3, ret_agg="mean"):
    df_panel["portfolio"] = df_panel.groupby("WeekYear")[dom_variable].transform(
        lambda x: pd.qcut(
            x, q=n_quantiles, labels=[f"P{i}" for i in range(1, n_quantiles + 1)]
        )
    )
...
def double_sort(
    df_panel, dom_variable, secondary_variable, n_quantiles=3, ret_agg="mean"
):
    ...
    df_panel["secondary_portfolio"] = df_panel.groupby(
        ...
        lambda x: pd.qcut(
            x, q=n_quantiles, labels=[f"Q{i}" for i in range(1, n_quantiles + 1)]
        )
    )
...
def vw_univariate_sort(df_panel, dom_variable, n_quantiles=3, ret_agg="mean"):
    df_panel["portfolio"] = df_panel.groupby("WeekYear")[dom_variable].transform(
        lambda x: pd.qcut(
            x, q=n_quantiles, labels=[f"P{i}" for i in range(1, n_quantiles + 1)]
        )
