...
def dropmissing(data: pd.DataFrame, event: str) -> pd.DataFrame:
    ...
    d = data.copy()
    logging.info(
        "Number of observations in input dataframe: %d", d.iloc[:, 0].count()
    )  # counts only the first column
    d = d[~(d[event].isna())]
    logging.info(
        "Number of observations in input dataframe: %d", d.iloc[:, 0].count()
    )  # counts only the first column
...
def balancepanel(
    ...
    event: str,
    calendartime: str,
    check_minmax: bool = True,
) -> bool:
    logging.info("\nChecks if input data frame is a BALANCED PANEL")
    d = data.copy()
    logging.info("Checking if every group has the same number of observations")
    check_N = d.groupby(group)[event].count()
    check_N = check_N.nunique()
    if check_N == 1:
        logging.info("Every group has the same number of observations")
    elif check_N > 1:
        logging.info("Some groups have different numbers of observations!")
    if check_minmax:  # calendartime is ignored if check_minmax = False
        ...
        if check_min == 1:
            logging.info("Every group has the same minimum calendartime value")
        elif check_min > 1:
...
def identifycontrols(data: pd.DataFrame, group: str, event: str) -> pd.DataFrame:
    print("\nIdentifying control groups: never-treated, or last-treated")
    print("Generates new indicator column: control_group")
    d = data.copy()
    list_group = list(
        d[group].unique()
    )  # generate iterable of all levels in the group column
    ...
    for (
        ...
        list_group
    ):  # finding if there are any groups with event = 0 for all calendartime
    ...
    if check_control == 1:
        ...
        for (
            ...
            list_group
        ):  # Quick hack: Last-treated groups have the least number of event = 1
            d.loc[d[group] == g, "_event_flagavg"] = d.loc[
                d[group] == g, event
            ].mean()  # average number of event flags by group
        min_event_flagavg = d[
            "_event_flagavg"
        ].min()  # find the global minimum avg event flag count
        d.loc[
            d["_event_flagavg"] == min_event_flagavg, "control_group"
        ] = 1  # check which group made the minimum avg event flag
        for g in list_group:  # make the control group indicator time-invariant by group
            d.loc[d[group] == g, "control_group"] = d.loc[
                d[group] == g, "control_group"
            ].max()
...
def genreltime(data, group, event, calendartime, reltime="reltime", check_balance=True):
    ...
    for (
        ...
        list_group
    ):  # create new temporary column of calendar time going from 0 to end of time
    ...
    for g in list_group:  # create new column with calendar - onset as integers
        try:
            d.loc[d[group] == g, reltime] = (
                d.loc[d[group] == g, "_ct"]
                - d.loc[((d[group] == g) & (d[event] == 1)), "_ct"].reset_index(
                    drop=True
                )[0]
            )
        except:
...
def gencohort(data, group, event, calendartime, cohort="cohort", check_balance=True):
    ...
    for g in list_group:
        try:
            d.loc[d[group] == g, cohort] = d.loc[
                ((d[group] == g) & (d[event] == 1)), "_ct"
...
def gencalendartime_numerics(
    data, group, event, calendartime, calendartime_numerics="ct"
):
        ...
        for z in list_Z:
            ...
            if (rho == 1) | (rho == np.nan):  # either collinear or invariant
                list_violate = list_violate + [
                    x
                ]  # drop x if precedence goes to later columns; drop z if otherwise
                list_Z.remove(
                    z
                )  # since dropped, remove from comparator list permanently
...
def checkfullrank(data, rhs, intercept="Intercept"):
    ...
    if intercept is None:
        ...
        rhs_reverse = [
            intercept
        ] + rhs_reverse  # move intercept back to the very front of the reversed list (prioritised)
    d = data[
        rhs_reverse
