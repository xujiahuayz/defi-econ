"""
Function to fetch the TVL data from the Uniswap V3 subgraph
"""
...
def _get_tvl_v3() -> None:
    """
    Function to get the TVL data from the Uniswap V3 subgraph
    """

    # request the data from the subgraph
    total_liquidity_json = run_query(HTTP_V3, tvl_query_v3)
    ...
    df_total_liquidity.to_csv(
        rf"{UNISWAP_V3_DATA_PATH}/tvl/total/total_liquidity.csv", index=False
    )

    # plot the total liquidity using matplotlib
    plt.figure(figsize=(20, 10))
    plt.plot(df_total_liquidity["date"], df_total_liquidity["tvlUSD"].map(float))
    plt.xlabel("Date")
    plt.ylabel("TVL (USD)")
    plt.title("Uniswap V3 Total Liquidity")
    plt.show()
...
def _get_primary_token() -> None:
    ...
    # Get the primary token pair set of Uniswap V3
    primary_token_pair_set_v3 = _get_primary_token_pair_set()
    ...
    # save the dateframe to rf"{UNISWAP_V3_DATA_PATH}/tvl/list"
    df_primary_token.to_csv(
        rf"{UNISWAP_V3_DATA_PATH}/tvl/list/primary_token.csv", index=False
    )
...
def _get_token_tvl(token_address: str, token_name: str, token_symbol) -> None:
    ...
    try:
        # set the params
        tvl_params_v3 = {"token_address": token_address}

        # query the graph
        tvl_json = run_query_var(HTTP_V3, tvl_query_v3, tvl_params_v3)
        ...
        # save the dateframe to rf"{UNISWAP_V3_DATA_PATH}/tvl/csv"
        df_tvl.to_csv(
            rf"{UNISWAP_V3_DATA_PATH}/tvl/csv/{token_symbol}.csv", index=False
        )
    except:
        print("Error: ", token_symbol)


def fetch_liquidity_v3() -> None:
    """
    Function to fetch the liquidity data from the Uniswap V3 subgraph
    """

    # check if there is path of data/data_uniswap_v3/tvl exists
    if not os.path.exists(rf"{UNISWAP_V3_DATA_PATH}/tvl"):

        # create the path
        os.makedirs(rf"{UNISWAP_V3_DATA_PATH}/tvl")
        os.makedirs(rf"{UNISWAP_V3_DATA_PATH}/tvl/csv")
        os.makedirs(rf"{UNISWAP_V3_DATA_PATH}/tvl/list")
    ...
    df_primary_token = pd.read_csv(
        rf"{UNISWAP_V3_DATA_PATH}/tvl/list/primary_token.csv"
    )

    # get the list of primary token address in {UNISWAP_V3_DATA_PATH}/tvl/csv
    primary_token_address_list = [
        filename.split(".")[0]
        for filename in os.listdir(rf"{UNISWAP_V3_DATA_PATH}/tvl/csv")
    ]

    # exclude the primary token address that already exists in {UNISWAP_V3_DATA_PATH}/tvl/csv
...
if __name__ == "__main__":
    # fetch_liquidity_v3()
    # _get_token_tvl("0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2", "WETH", "WETH")
    _get_tvl_v3()
